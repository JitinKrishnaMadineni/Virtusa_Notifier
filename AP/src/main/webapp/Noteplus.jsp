...
try
{
	...
	Class.forName("com.mysql.jdbc.Driver");
	Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/mydbvirtusa","root","");
	PreparedStatement stmt=con.prepareStatement("select Notebookname,Notename,Description,Startdate,Status from note where Email=? and Notebookname=?");
	stmt.setString(1,eemail);
	stmt.setString(2,request.getParameter("notebookname2"));
	ResultSet rs=stmt.executeQuery();
	...
	{ 
		...
		//session.setAttribute("notebookname",rs.getString("Notebookname"));
			    ...
			    out.println("<li class='notes'><a href='NotePlusDisplay.jsp?notebookname="+rs.getString("Notename")+"' style='padding:5px;'>"+rs.getString("Notename")+" </a></li><li class='plus'><br><a href='notes1.jsp?notebookname="+request.getParameter("notebookname2")+"'><b>+</b></a><br><b>"+rs.getString("Description")+"</b>"+"</a>"
			    		+"<span><i style='float:center;padding:0px 50px;'>Started On:"+rs.getString("Startdate")+"</i>"
			    //+"<i  style='float:center;padding:0px 30px;'>"+rs.getString("Status")+"</i></span>"
			    		  +"<input type='submit' class='submit' value="+rs.getString("Status")+"></span>"
			    		+"<a href='DeleteNoteNote.jsp?notename="+rs.getString("Notename")+"&notebookname="+rs.getString("Notebookname")+"' style='float:right;padding:0px 20px;'><b>&times</b>Delete</a>"
			    +"<a href='EditNoteNote.jsp?notename="+rs.getString("Notename")+"&notebookname="+rs.getString("Notebookname")+"' style='float:right;'><i class='fa fa-edit' style='font-size:14px'></i>Edit</a></li>"
